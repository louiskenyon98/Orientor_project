...
from app.models.course import Course, ConversationLog
from app.schemas.course import TargetedAnalysisRequest

class TestLLMCourseService:
    """Test suite for LLMCourseService."""
    ...
    @pytest.fixture
    def sample_context(self, test_course):
        """Sample analysis context."""
        return {
            "course": {
                "name": test_course.course_name,
                "category": test_course.subject_category,
                "grade": test_course.grade,
    ...
    @pytest.mark.asyncio
    async def test_generate_targeted_questions_success(self, llm_service, mock_openai_client, sample_context):
        """Test successful targeted question generation."""
        ...
        mock_response.choices[0].message.content = json.dumps([
            ...
            },
            {
                ...
                "follow_up_triggers": ["prefer", "approach", "method"]
            }
        ])
        
        mock_openai_client.chat.completions.create.return_value = mock_response
        
        with patch('app.services.llm_course_service.openai_client', mock_openai_client):
            ...
            assert questions[1]["intent"] == "cognitive_preference"

    @pytest.mark.asyncio
    async def test_generate_targeted_questions_invalid_json(self, llm_service, mock_openai_client, sample_context):
    ...
    @pytest.mark.asyncio
    async def test_analyze_response_success(self, llm_service, mock_openai_client):
    ...
    @pytest.mark.asyncio
    async def test_analyze_response_openai_error(self, llm_service, mock_openai_client):
    ...
    @pytest.mark.asyncio
    async def test_process_conversation_turn(self, llm_service, mock_openai_client, db_session, test_user, test_course):
        """Test processing a complete conversation turn."""
        session_id = "test_session_123"
        user_response = "I really enjoyed the analytical problem-solving aspects of this course."
        ...
        with patch('app.services.llm_course_service.openai_client', mock_openai_client):
            result = await llm_service.process_conversation_turn(
                ...
                user=test_user,
                db=db_session,
                focus_areas=["cognitive_preference"]
            )
            
            assert "analysis" in result
            assert "next_question" in result
            assert result["analysis"]["insights"][0]["type"] == "cognitive_preference"
            assert result["next_question"]["intent"] == "cognitive_preference"
            
            # Verify conversation log was created
            log = db_session.query(ConversationLog).filter_by(session_id=session_id).first()
            assert log is not None
            assert log.response == user_response
            assert log.question_intent == question_intent